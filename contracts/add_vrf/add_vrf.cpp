...
const std::string verify_vrf_name        = "verify_vrf_proof";
...
int main()
{
   ...
   {
      system::log( "verify_vrf_proof cost has already been added to compute bandwidth registry" );
   ...
   {
      system::log( "could not find compute bandwidth registry" );
      system::exit_contract( 1 );
   }

   chain::compute_bandwidth_entry< 64 > entry;
   entry.mutable_name().set( constants::verify_vrf_name.data(), constants::verify_vrf_name.size() );
   entry.set_compute( 143804 );
